(function ($, window, document) {
    var mid = 0;
    //1.7.0版本才有$.now，$.isEmptyObject，$.later函数，这里临时修补下
    $.now = $.now ? $.now : function () { return +new Date(); }
    $.isEmptyObject = $.isEmptyObject ? $.isEmptyObject : function (o) {
        for (var p in o) {
            if (p !== undefined) {
                return false;
            }
    ...
    $.later = $.later ? $.later : function (fn, when, context, data) {
        when = when || 0;
        var m = fn;
        var d = data;
        var f;
